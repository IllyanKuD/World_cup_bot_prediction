...
class DataStore:
    def __init__(self, data_dir: str = DATA_DIR):
        ...
        self.matches = pd.read_csv(f"{d}/matches.csv", parse_dates=["date"])
        self.mts = pd.read_csv(f"{d}/match_team_stats.csv")
        self.events = pd.read_csv(f"{d}/match_events.csv")
        ...
        # -- lookup tolerant (accents / casse) pour equipes et joueurs --
    ...
    def _train_poisson_model(self):
        X = self._long[self.FEATURE_COLS].values
        y = self._long["goals_for"].values
        scaler = StandardScaler().fit(X)
        Xs = scaler.transform(X)
        model = PoissonRegressor(alpha=1.0, max_iter=2000)
        model.fit(Xs, y)
        return model, scaler, self.FEATURE_COLS

    def _train_logit_model(self):
        f = self.features.dropna(subset=["match_result"]).copy()
