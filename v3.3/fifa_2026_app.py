...
class DataStore:
    def __init__(self, data_dir: str = DATA_DIR):
        ...
        self.player_stats = self._augment_player_stats_with_manual_matches(self.player_stats)
    ...
    def _augment_player_stats_with_manual_matches(self, player_stats: pd.DataFrame) -> pd.DataFrame:
        """Ajoute les buts inscrits lors des matchs saisis a la main
        (manual_results.csv) aux stats agregees des joueurs, pour que 'Vue
        d'ensemble' (top buteurs), 'Statistiques equipe' (meilleurs joueurs)
        et 'Statistiques joueur' refletent bien les matchs saisis a la main.

        LIMITE : la saisie manuelle ne capture que les buteurs (pas les
        compositions completes, passes decisives, cartons ou minutes
        jouees). Seul le compteur `goals` est donc corrige ici ; les autres
        compteurs (assists, matches_played, cartons, minutes...) restent
        ceux du dataset d'origine et ne peuvent pas etre deduits d'une
        simple saisie de score."""
        ...
        if not goal_counts or "player_id" not in player_stats.columns:
            return player_stats

        ps = player_stats.set_index("player_id")
        for pid, n_goals in goal_counts.items():
            if pid in ps.index:
                ps.loc[pid, "goals"] = ps.loc[pid, "goals"] + n_goals
        return ps.reset_index()
